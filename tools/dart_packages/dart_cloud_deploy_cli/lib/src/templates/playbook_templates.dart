...
''';
  ...
  /// Container registry deployment playbook template
  static String containerRegistry({
    String appDir = '/opt/dart_cloud',
    String registryUrl = '',
    String registryUsername = '',
    String registryTokenBase64 = '',
    String imageName = '',
    String imageTag = 'latest',
    String containerRuntime = 'podman',
    String projectName = 'dart_cloud',
    String serviceName = 'backend-cloud',
    String composeFile = 'docker-compose.yml',
  }) =>
      '''
---
# Ansible Playbook: Container Registry Deployment
# Generated by dart_cloud_deploy CLI

- name: Deploy Container from Registry
  hosts: dart_cloud_servers
  become: true
  vars:
    app_dir: $appDir
    ...
    registry_token_base64: $registryTokenBase64
  ...
  tasks:
    ...
    - name: Login to container registry
      ansible.builtin.shell: |
        echo "{{ registry_token }}" | {{ container_runtime }} login {{ registry_url }} --username {{ registry_username }} --password-stdin
      no_log: true
      changed_when: false
      tags:
        - registry
    ...
    - name: Pull container image from registry
      ansible.builtin.shell: |
        {{ container_runtime }} pull {{ registry_url }}/{{ image_name }}:{{ image_tag }}
    ...
    - name: Stop existing container
      ansible.builtin.shell: |
        {{ container_runtime }}-compose -p {{ project_name }} -f {{ compose_file }} stop {{ service_name }} || true
      args:
        chdir: "{{ app_dir }}"
      changed_when: false
      tags:
    ...
    - name: Remove existing container
      ansible.builtin.shell: |
        {{ container_runtime }} rm -f {{ project_name }}_{{ service_name }} || true
      args:
        chdir: "{{ app_dir }}"
      changed_when: false
      tags:
    ...
    - name: Update docker-compose to use registry image
      ansible.builtin.lineinfile:
        ...
        regexp: '^(\\s+)image:.*{{ service_name }}.*'
        line: '    image: {{ registry_url }}/{{ image_name }}:{{ image_tag }}'
        backrefs: yes
      tags:
        - config
    ...
    - name: Start container with new image
      ansible.builtin.shell: |
        {{ container_runtime }}-compose -p {{ project_name }} -f {{ compose_file }} up -d {{ service_name }}
      args:
        chdir: "{{ app_dir }}"
    ...
    - name: Logout from container registry
      ansible.builtin.shell: |
        {{ container_runtime }} logout {{ registry_url }}
      changed_when: false
      tags:
        - registry
        - logout
    ...
    - name: Display deployment status
      ansible.builtin.debug:
        msg: "Container deployed successfully from registry: {{ registry_url }}/{{ image_name }}:{{ image_tag }}"
      tags:
        - deploy
''';
}
